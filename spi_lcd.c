...
unsigned char getColorIndexFromFrameBuffer(int x, int y, uint8_t* pixels)
{
	int offset = x + y * 160;
    ...
    for (y=0; y<height; y++) {
        ...
        while (x<width) {
            for (i=0; i<16; i++) {
				...
				//temp[i] = palette[getColorIndexFromFrameBuffer(x, y, data)]; x++;
                x1 = palette[getColorIndexFromFrameBuffer(x, y, data)]; x++;
                y1 = palette[getColorIndexFromFrameBuffer(x, y, data)]; x++;
