...
void ili9341_write_frame(const uint16_t xs, const uint16_t ys, const uint16_t width, const uint16_t height, const uint8_t* data, bool usepalette) {
	int x, y, offset;
	int i;
	uint16_t x1, y1;
	uint32_t xv, yv, dc;
	uint32_t temp[16];
	const uint16_t* rawpixels = (const uint16_t*)data;
	dc = (1 << PIN_NUM_DC);
	offset = 0;
	...
	for (y=0; y<height; y++) {
		...
		while (x<width) {
			for (i=0; i<16; i++) {
				if (usepalette) {
					x1 = palette[data[offset++]];
					y1 = palette[data[offset++]];
				}
				else {
					x1 = rawpixels[offset++];
					y1 = rawpixels[offset++];
